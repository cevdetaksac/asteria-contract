...
| **Guardian** (≥4.6.0) | **Windows Servisi** `CloudHoneypotGuardian` (LocalSystem, Automatic, SCM restart-on-failure) | **Yalnız watchdog** — motoru diriltir, kendisi motoru çalıştırmaz. Motor ↔ Guardian çapraz korur ([`../features/persistence.md`](../features/persistence.md)) |
...
**Durdurma politikası (≥4.6.0):** Motor+Guardian yalnız (1) güncelleme (`update_in_progress.lock`) veya (2) PIN ile imzalı `operator_stop.json` ile durur. Başka her sonlanma → tamper → diriliş + `agent_tamper` alarmı. Detay: [`../features/persistence.md`](../features/persistence.md).
