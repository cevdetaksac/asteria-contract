...
> **SoT** **≥ 1.4.61**. Pointer: [`../agent/network-guard.md`](../agent/network-guard.md)

Detection is **always alert-only**. Cloud cannot enable `auto_contain` / `auto_kill` /
`auto_restore` as silent kill. Suspend only via confirmed `suspend_process` (exact pid).
